...
- **Always check Expo Go compatibility BEFORE picking a third-party library.** Expo Go is a pre-built app with a fixed set of native modules. Any library requiring custom native code outside that set will crash at runtime with no clear error. The question to ask first: "Does this library need native code, and is that native code included in Expo Go?" Confirmed included in SDK 54: `react-native-webview`, `react-native-svg`. If unsure, check docs.expo.dev or use `npx expo install` which warns about incompatibilities. Failing to do this caused 5 failed math rendering attempts in a row (react-native-mathjax, MathJax WebView CDN, react-native-math-view, KaTeX WebView) before landing on the right solution.
- **Library source code is the ground truth — read it before using it.** `react-native-mathjax` had a broken CDN URL and silently ignored the `html` prop we were passing. We only discovered this by reading `node_modules/react-native-mathjax/index.js`. Don't trust documentation or npm descriptions alone. When a library behaves unexpectedly, read its source first.
